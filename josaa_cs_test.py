# -*- coding: utf-8 -*-
"""
Created on Wed Oct 13 17:26:04 2021
...
gc = pyg.authorize(service_file = r"C:\Users\Praneeth Ponnekanti\Documents\josaacounsellor-creds.json")
...
r1 = pd.read_csv(r"C:\Users\Praneeth Ponnekanti\Documents\Josaa_R1.csv")
r2 = pd.read_csv(r"C:\Users\Praneeth Ponnekanti\Documents\Josaa_R2.csv")
r3 = pd.read_csv(r"C:\Users\Praneeth Ponnekanti\Documents\Josaa_R3.csv")
r4 = pd.read_csv(r"C:\Users\Praneeth Ponnekanti\Documents\Josaa_R4.csv")
r5 = pd.read_csv(r"C:\Users\Praneeth Ponnekanti\Documents\Josaa_R5.csv")
r6 = pd.read_csv(r"C:\Users\Praneeth Ponnekanti\Documents\Josaa_R6.csv")
college_states = pd.read_csv(r"C:\Users\Praneeth Ponnekanti\Documents\Josaa_College_States.csv")
domicile_states = pd.read_csv(r"C:\Users\Praneeth Ponnekanti\Documents\josaa_domicile_states.csv")
josaa_business_rules = pd.read_csv(r"C:\Users\Praneeth Ponnekanti\Documents\josaa_business_rules.csv")
course_df = pd.read_csv(r"C:\Users\Praneeth Ponnekanti\Documents\josaa_courses.csv")
...
def search_results(jee_rank,rank_var,inp_st,df,round_no,inst_bucket,d_state,locations):
    ...
    result_hs_df = result_df[(result_df['Location'] == d_state) & (result_df['Quota'] == 'HS')]
...
def download_table(df,name):
    ...
    #print("b64", b64)
    st.markdown("#### Download the above table as a csv file ###")
    href = f'<a href="data:file/csv;base64,{b64}" download = "{op_file}">Download csv file</a>'
    st.markdown(href,unsafe_allow_html=True)
    #st.success("Downloading of file : " + op_file + " is completed.")
    return
    
def fetch_credentials():
    gc_c = gc.open("JEESaathi_Credentials")
    wks_c = gc_c[0]
    cred_df = wks_c.get_as_df(has_header = True)
    return cred_df

def send_email(user_id,csv,attachment,mail_body):
    c_df = fetch_credentials()
    fromaddr = c_df[['UserName']]
    toaddr = user_id
    if user_id is '':
    ...
    # instance of MIMEMultipart
        ...
        # storing the subject 
        msg['Subject'] = "Your JEE College Predictor results are here !"
        ...
        # Authentication
        s.login(fromaddr, c_df[['Password']])
...
unwanted_engg_mains_inst = {'IIT','SPA'}
inst_type = [ele for ele in inst_type if ele not in unwanted_engg_mains_inst]
...
_inp_rank = st.number_input("Type in your JOSAA category rank to search (CRL Rank for OPEN category). Default rank value is 1. ", min_value = 1,key = 'rank')
...
inp_loc_selector = st.sidebar.multiselect("Select Preferred states : ", master_df['Location'].unique(), default = list(master_df['Location'].unique()))
...
#or not _inp_rank.isnumeric():
    ...
    st.write("Your Rank is " + str(_inp_rank) +" in " + str(inp_category) + " category, Your domicile state is " + inp_dom_state +".")
    dom_eq_hs = eq_hs_nit(inp_dom_state,domicile_states)
    ...
    if col_b1.button("Submit and Search"):
        ...
        c_result_df = search_results(_inp_rank,inp_rank_var,inp_category,master_df,inp_round,inp_inst_type,dom_eq_hs,inp_loc_selector)
        
        col5,col6 = st.columns(2)
        
        #if(col5.button("Download the table results as an excel file.", on_click= download_table())):
        op_file = download_table(c_result_df,name)
