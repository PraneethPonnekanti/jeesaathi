...
def download_table(df,name):
    ...
    f'<a href="data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,{base64.b64encode(open(op_file, "rb").read()).decode()}" download="{op_file}">Click here to download the table results in an Excel file!</a>'
    #timestr = time.strf()
    #csv = df.to_csv(op_file,index=False)
